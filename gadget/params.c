...
create_gadget_parameter_set()
{
    ...
    param_declare_double(ps, "CritPhysDensity", OPTIONAL, 0, "Threshold physical density (in protons/cm^3) for gas to be star forming. If zero, calculate from SH03 Eq.23.");
    param_declare_double(ps, "PhysDensThreshFactor", OPTIONAL, 1, "If CritPhysDensity is set to 0, it will be calculated based on SH03 Eq.23 and multiply by this factor");                  
